...
串口桥最容易做偏的地方，是"顺手"帮你解析协议。我调研时见过的反例就是 JSON+base64 包帧、桥内做 Modbus：桥一旦做语义，页面侧协议栈就得跟着它的帧格式重写，接入那天就是推翻自家代码的一天。SerialHub 把定位钉死在"管道"：`/ws` 数据通道只传**原始二进制帧**，双向；组帧、解析、校验全部留给页面和脚本自己的协议栈。配置与状态走 `/api/*`（JSON），控制面与数据面彻底分离，管理台页面挂了也不影响字节流动。
...
两个示例都在仓库 examples/ 目录，拿来改改就能用。注意**管理台端口 ≠ 数据端口**，真实数据网址以桥卡片或启动输出为准。
