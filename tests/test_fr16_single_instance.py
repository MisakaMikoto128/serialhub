# -*- coding: utf-8 -*-
"""Sprint 8 QA — FR-16 单实例友好处理 (decisions.md ADR-19① / spec FR-16)。

契约 (spec FR-16 + 任务契约, 黑盒只认可观察行为):
- 第二实例 bind 失败时探测目标端口 /api/status (响应含 "phase" 判为另一 SerialHub):
  * 是 SerialHub → headless: stderr 含 "已在运行" 且以 0 退出;
    GUI: 信息框 (非错误) + 自动在浏览器打开管理台, 以 0 退出;
...
- `fr16_gate` 会话级行为探针: 对每个构建实测 "第二实例 exit==0?"。
  ...
  非 SerialHub 占用组不设门 —— v1.3.0 基线已是 exit 1 + 报占用 (2026-09-13 实测),
  落地前后都必须成立 (守护"旧错误路径不被误伤")。

GUI 进程级说明: 信息框/错误框为 Win32 MessageBox (阻塞), 自动化以 WM_CLOSE 关闭
(单键对话框等价"确定"); 弹窗文案与"自动开浏览器"属目视项, 见
docs/team/reports/qa-sprint8-plan.md 人工清单。GUI exit-0 用例会真实打开一个浏览器
标签 (确定性副作用, ADR-19① 契约如此); 无显示环境可设 SERIALHUB_QA_SKIP_GUI=1 跳过。
...
# ADR-19① 契约文案的判别子串 (headless stderr: "SerialHub 已在运行"; 取宽松核心,
# "管理台: <网址>" 等前后缀不锁, 避免过度约束实现)
FRIENDLY_MARK = "已在运行"
OCCUPIED_MARK = "占用"
...
@pytest.mark.parametrize("exe", BUILD_PARAMS)
def test_fr16c_gui_second_instance_info_box_exit0(exe, fr16_gate, instances):
    """GUI 第二实例 (目标=另一 SerialHub): 信息框 → 确定 → 开管理台 → exit 0。

    自动化只断进程级退出码 0 + A 仍活; 信息框为 Win32 MessageBox (阻塞),
    用 WM_CLOSE 关闭 (单键对话框等价"确定")。弹窗文案/图标与"浏览器自动打开
    管理台"属目视项 (人工清单见 qa-sprint8-plan.md §4); 确定后浏览器会真实
    打开管理台标签 —— 本用例的确定性副作用。无显示环境设
    SERIALHUB_QA_SKIP_GUI=1 跳过。
    """
    ...
    gui = Instance(exe, addr_args(port))   # 不带 --headless = GUI 默认形态
    instances.append(gui)
    dismiss_msgbox_of_pid(gui.proc.pid, timeout=20.0)   # WM_CLOSE = 确定 (单键框)

    deadline = time.monotonic() + 20
    while gui.alive and time.monotonic() < deadline:
        time.sleep(0.2)
    assert not gui.alive, "GUI 第二实例确定信息框后 20s 内未退出"
    assert gui.proc.returncode == 0, (
        f"FR-16 违约: GUI 第二实例 (目标为 SerialHub) 应 0 退出, "
        f"实得 {gui.proc.returncode}")
    assert a.alive, "GUI 第二实例退出后第一实例 A 竟退出 (不得误伤)"
...
def dismiss_msgbox_of_pid(pid: int, timeout: float = 20.0) -> None:
    """等 pid 的可见 MessageBox (对话框类 #32770) 出现并以 WM_CLOSE 关闭。
    ...
    while time.monotonic() < deadline:
        ...
        if hits:
            user32.PostMessageW(hits[0], 0x0010, 0, 0)   # WM_CLOSE
            return
        time.sleep(0.2)
    raise AssertionError(
