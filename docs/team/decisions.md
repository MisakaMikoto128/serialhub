...
② 站点托管 = 双站并行, 用平台默认子域 (不购域名, 未来购域名一键 CNAME): **GitHub Pages** 为
   国际主站 (与仓库/CI 同源) + **Cloudflare Pages 或 EdgeOne Pages** 国内可达镜像 (部署实测后
   二选一定稿, 免费档均够用)。
③ 安装包双通道: 国际 = **GitHub Releases** —— release.yml 已自动化三平台打包上传, 零新工作;
   国内 = **蓝奏云** 官方直传镜像 (免费, 单文件 ≤100MB 量级, 包体 ~10MB 远够)。站点只做导航
   不中转文件, 版本事实来源仍是 tag; 站点公布 **SHA256** (发布工程组自 CI 产物核算) 供校验。
④ 原创红线: 只借鉴信息架构模式; 文案/配色/图形/代码全部原创, 验收含与参考站逐区块非相似核对
   (WEB-3/4)。差异化走产品自己的资产: 真实截图 + FR-15 图标母题 + 可选 win95 彩蛋。
⑤ 用户侧前置 (免费但需注册): 蓝奏云账号 (国内通道); Cloudflare 或 EdgeOne 账号 (镜像站);
   GitHub Pages 免费档要求仓库公开 (ADR-17 既定公开)。
