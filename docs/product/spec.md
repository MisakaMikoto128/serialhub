...
- **FR-18 发布版无控制台**: Windows release 构建为 GUI 子系统 (无控制台窗口);
  调试构建保留; headless release 的 stdout 不可见属既定取舍 (手册注明)。
- **FR-15 应用图标**: SVG 主标 (桥+串口母题, 16px 可辨) 输出 全尺寸 ico/png/favicon;
  exe 内嵌 (winres), 桌面窗口/托盘 (运行中绿·重连中琥珀·已停止灰 三态变体) / 网页
  favicon 全部生效。

- **FR-16 单实例友好处理 (ADR-25 改版)**: 第二实例 bind 冲突时探测目标端口 ——
  若为另一 SerialHub (status 形状判别) → 向其 **POST /api/show** 令已运行
  实例把主窗口拉到前台 (set_visible + 取消最小化 + 抢焦点), 第二实例**静默以
  0 退出** (无提示框、不开浏览器, "重复点图标 = 把软件叫回来"; headless 对端
  无窗口 → {"ok":true,"shown":false}, 第二实例同样静默 0 退出, stderr 一行
  "已在运行"); 被其他程序占用 → 维持现有错误框 (FR-8)。
